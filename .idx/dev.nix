...
{ pkgs, ... }: {
  ...
  env = {
    MOBILE_PATH = "$WORKSPACE_ROOT/mobile";
  };
  idx = {
      ...
      onCreate = {
        build-flutter = ''
          ...
          ./gradlew \
            ...
            -Ptarget=$MOBILE_PATH/lib/main.dart \
    ...
    previews = {
      ...
      previews = {
        web = {
          command = ["flutter" "run" "--machine" "-d" "web-server" "--web-hostname" "0.0.0.0" "--web-port" "$PORT" "--project-path" "$MOBILE_PATH"];
          manager = "flutter";
        };
        android = {
          command = ["flutter" "run" "--machine" "-d" "android" "-d" "localhost:5555" "--project-path" "$MOBILE_PATH"];
          manager = "flutter";
        };
      };
    };
